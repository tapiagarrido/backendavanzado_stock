...
export class UpdateMaestraArticuloDto {
    ...
    static create(object: { [key: string]: any }): [string?, UpdateMaestraArticuloDto?] {

        const { id, nombre, codigo_barra, marca, descripcion, valor, usuario_id, tipo_id, linea_id, local_id, unidad_medida_venta_id, updated_at, categoria_id } = object;

        if (!id) return ["No se encuentra el id del articulo"];
        ...
        return [undefined, new UpdateMaestraArticuloDto(id, nombre, codigo_barra, marca, descripcion, valor, usuario_id, tipo_id, linea_id, local_id, unidad_medida_venta_id, updated_at, categoria_id)]
    }
}